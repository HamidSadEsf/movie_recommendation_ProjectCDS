...
selection_of_users = random.sample(final_df["userId"].value_counts().index.to_list(), 1000)
...
pivot.to_csv('./data/processed/pivot.csv', index=True, header="userId")

#Create non-sparse dataset
csr_data = csr_matrix(pivot.values)
save_npz("./data/processed/csr_ratings.npz", csr_data)
